  ...
  return (
    <div className="space-y-6 py-4">
      <div className="flex flex-col items-center text-center justify-center space-y-2">
        <h2 className="text-2xl text-black font-semibold w-96">Your account has been created! What brings you to GoPratle?</h2>
        <p className="text-orange-500 font-semibold py-6">We want to tailor your experience so you'll feel right at home.</p>
      </div>
        ...
        {[ 
          ...
          >
            <div className="flex flex-col text-left items-start gap-4 w-full">
              <img src={option.image} alt={option.title} className="h-20 w-20" />
              <h3 className="text-xl w-full font-semibold text-orange-500">{option.title}</h3>
            </div>
          </button>
        ))}
      </div>
